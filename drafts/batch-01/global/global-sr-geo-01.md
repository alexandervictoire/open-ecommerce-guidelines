...
- Delivery to another member state is declined without saying so until checkout.
...
5. Where delivery is not offered to that state, confirm it is stated before the user invests effort, ideally at the cart.
...
Recommend a storefront rather than imposing one, persist the visitor's own choice above any inference, keep every storefront reachable by direct URL, and state delivery limitations at the cart rather than at checkout.
