...
//
// Only listed guidelines count (published and draft, see LISTED_STATUSES in
// utils/labels.ts), so a category holding nothing but deprecated guidelines
// does not produce an empty page.
function listRoutes(): string[] {
  ...
  for (const entry of readdirSync(root)) {
    ...
    for (const file of readdirSync(dir).filter((f) => f.endsWith('.md'))) {
      ...
      if (!status || !listed.includes(status)) continue
      hasListed = true
      const dimension = /^dimension:[ \t]*(\S+)[ \t]*$/m.exec(text)?.[1]
      if (dimension) dimensions.add(dimension)
    }
    if (hasListed) categories.push(`/guidelines/${entry}/`)
  }
